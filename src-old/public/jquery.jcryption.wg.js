...
(function($) {
	...
	/**
	 * Modifications pour WG SoHo
	 * - Logs
	 */
	$.jCryption.authenticate = function(AESEncryptionKey, publicKeyURL, handshakeURL, success, failure, stateChange) {
		console.log('   jCryption: authenticate');

		// 1) Client generates a Password (mousemovement of user used when possible) --> AESEncryptionKey

		if (stateChange) stateChange($.jCryption.state.REQUEST_RSA_PUBLIC_KEY);
		...
		$.jCryption.getKeys(publicKeyURL, function(keys) {
			...
			$.jCryption.encryptKey(AESEncryptionKey, keys, function(encryptedKey) {
				console.log('   jCryption: encryptKey');

				if (stateChange) stateChange($.jCryption.state.HANDSHAKE);

				// 3) And send it to the server
				$.jCryption.handshake(handshakeURL, encryptedKey, function (response) {

					// 4) Server decrypts Password with RSA Private key and stores it in the session
					// 5) Server encrypts the Password with AES using the password as encryption key, and sends it back to the Client

					// 6) Client decrypts it with AES using the password
					if ($.jCryption.challenge(response.challenge, AESEncryptionKey)) {
						...
						console.log('   jCryption: handshake success');
						if (stateChange) stateChange($.jCryption.state.HANDSHAKE_SUCCESS);
						success.call(this, AESEncryptionKey);
					} else {
						console.log('   jCryption: handshake failure');
						if (stateChange) stateChange($.jCryption.state.HANDSHAKE_FAILURE);
						failure.call(this);
					}
				});
			});
	...
	$.jCryption.challenge = function(challenge, key) {
		var xkey = $.jCryption.decrypt(challenge, key);
		if (xkey == key) {
			return true;
		}
		console.log('   jCryption: challenge failure ('+xkey+' != '+key+')');
		return false;
	};
	...
	$.jCryption.handshake = function(url, key, callback) {
		$.ajax({
			...
			error: function () {
				console.log('   jCryption: ajax failure in handshake');
			}
		});
	};
