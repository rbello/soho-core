...
*/
 ...
 $DBDEF = array(
	'user'=>	WG::vars('db_user'),#required
	'pwd'=>		WG::vars('db_pwd'), #required
	'db'=>		WG::vars('db_name'), #optional, default DB
	'host'=>	WG::vars('db_host'),#optional
	'port'=>	"",#optional
...
//constants
 ...
 $xurl = 'XSS='.$_SESSION['XSS'].'&w=mysql-adminer';
...
//strip quotes if they set
 ...
 if (array_key_exists('login', $_REQUEST)){
    if ($_REQUEST['pwd']!=$ACCESS_PWD){
       $_SESSION['err_msg']="Invalid password. Try again";
    }else{
       $_SESSION['is_logged']=true;
       loadcfg();
    }
 ...
 if (array_key_exists('logoff', $_REQUEST)){
    check_xss();
    $_SESSION = array();
    savecfg();
    session_destroy();
 ...
 if (array_key_exists('savecfg', $_REQUEST)){
    check_xss();
    savecfg();
 }
 ...
 if (array_key_exists('showcfg', $_REQUEST)){
    print_cfg();
    exit;
 }

//get initial values
$SQLq= array_key_exists('q', $_REQUEST) ? trim($_REQUEST['q']) : '';
$page= array_key_exists('p', $_REQUEST) ? intval($_REQUEST['p']) : 0;
if (array_key_exists('refresh', $_REQUEST) && $_REQUEST['refresh'] && array_key_exists('db', $DB) && $DB['db'] && preg_match('/^show/',$SQLq) ) $SQLq="show tables";
...
if (db_connect('nodie')){
	...
	if (array_key_exists('db', $_REQUEST)){
		if (array_key_exists('shex', $_REQUEST)){
			print_export();
		}
		if (array_key_exists('doex', $_REQUEST)){
			check_xss();
			do_export();
		}
		if (array_key_exists('shim', $_REQUEST)){
			print_import();
		}
		if (array_key_exists('doim', $_REQUEST)){
			check_xss();
			do_import();
		}
		if (array_key_exists('dosht', $_REQUEST)){
			check_xss();
			do_sht();
		}
		elseif (!array_key_exists('refresh', $_REQUEST) || preg_match('/^select|show|explain|desc/i',$SQLq) ){
			check_xss();
		...
		elseif (preg_match('/^show\s+(?:databases|status|variables|process)/i',$SQLq)){
			...
			$_SESSION['err_msg']="Select Database first";
		}
	}
...
function display_select($sth,$q){
 ...
 $w="width='100%' ";
 if ($is_sht || $is_shd) {$w='';
   $url='?XSS='.$_SESSION['XSS']."&w=mysql-adminer&db=$dbn";
   $sqldr.="<div class='dot'>
&nbsp;MySQL Server:
 ...
 while($row=mysql_fetch_row($sth)){
   ...
   for($i=0;$i<$fields_num;$i++){
      ...
      if ($is_sht && $i==0 && $v){
         ...
         $url='?XSS='.$_SESSION['XSS']."&w=mysql-adminer&db=$v";
...
function print_header(){
 global $VERSION,$DB,$dbh,$self,$is_sht,$xurl,$DBDEF;
 $dbn= array_key_exists('db', $DB) ? $DB['db'] : $DBDEF;
?>
...
<input type="hidden" name="w" value="mysql-adminer">
...
echo $xurl?>&q=show+databases">Databases</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option><?php echo get_db_select($dbn)?></select>
...
<div class="err"><?php echo $_SESSION['err_msg']; ?></div>

<?php
}

function print_screen(){
 global $out_message, $SQLq, $reccount, $time_all, $sqldr, $page, $MAX_ROWS_PER_PAGE, $is_limited_sql;
...
function db_connect($nodie=0){
 ...
 if (!$dbh) {
    $_SESSION['err_msg']='Cannot connect to the database because: '.mysql_error();
    if (!$nodie) die($_SESSION['err_msg']);
 }

 if ($dbh && $DB['db']) {
  $res=mysql_select_db($DB['db'], $dbh);
  if (!$res) {
     $_SESSION['err_msg']='Cannot select db because: '.mysql_error();
     if (!$nodie) die($_SESSION['err_msg']);
  }else{
 ...
 // Load database from Workgroop
 $dbh = WG::database()->resource;
 $DB['db'] = $DBDEF;
...
function get_db_select($sel=''){
 global $DB;
 if (array_key_exists('sql_sd', $_SESSION) && is_array($_SESSION['sql_sd']) && $_REQUEST['db']!='*'){//check cache
    $arr=$_SESSION['sql_sd'];
 }else{
...
function loadsess(){
 ...
 $rdb=array_key_exists('db', $_REQUEST) ? $_REQUEST['db'] : '';
 if ($rdb=='*') $rdb='';
 if ($rdb) {
    $DB['db']=$rdb;
 }
 ...
 if ($F && $F['name']){
  ...
  if ($pi['extension']!='sql'){//if not sql - assume .gz
     ...
     if (($gz=gzopen($filename,'rb')) && ($tf=fopen($tmpf,'wb'))){
        while(!gzeof($gz)){
           if (fwrite($tf,gzread($gz,8192),8192)===FALSE){$_SESSION['err_msg']='Error during gz file extraction to tmp file';break;}
        }//extract to tmp file
        gzclose($gz);fclose($tf);$filename=$tmpf;
     }else{$_SESSION['err_msg']='Error opening gz file';}
  }
  if (!$_SESSION['err_msg']){
   if (!do_multi_sql('', $filename)){
      $_SESSION['err_msg']='Import Error: '.mysql_error($dbh);
   }else{
  ...
  $_SESSION['err_msg']="Error: Please select file first";
 }
 print_import();
 exit;
}
...
function get_rand_str($len){
 ...
 for($i=0;$i<$len;$i++) $result .= $chars[rand(1,count($chars))-1];
 return $result;
}
